...
$pl_snippet_list = PL_Shortcode_CPT::template_list($shortcode, false, true);
$pl_snippet_types = array('default' => 'Built-In', 'custom' => 'Custom'); // Order matters, here...
...
<section class="shortcode_ref">
	...
	<?php if( ! empty( $select_name ) ) { echo 'name="'. $select_name . '"'; } ?>>
		<option value="" class=""<?php echo $value == '' ? 'selected="selected"' : '' ?>>
			default
		</option>
		...
		<?php for ($i=0; $i<=$count; ): ?>
			...
			<?php if($curr_type != $template['type']):?>
				...
				<?php if(!empty($pl_snippet_types[$curr_type])):?>
					<optgroup label="<?php echo $pl_snippet_types[$curr_type] ?>" data-type="<?php echo $curr_type ?>">
				<?php endif ?>
			<?php endif ?>

			<option value="<?php echo $id ?>" class="<?php echo $curr_type ?>" <?php echo $value == $id ? 'selected="selected"' : '' ?>>
