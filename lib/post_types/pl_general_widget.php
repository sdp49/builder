...
// 							), 'css' => 'pl_map' ),
	...
	public function meta_box() {
		add_meta_box( 'pl-controls-metabox-id', 'Create Shortcode', array( $this, 'pl_widgets_shortcode_meta_box_cb'), 'pl_general_widget', 'normal', 'high' );
	...
	public  function pl_widgets_shortcode_meta_box_cb( $post ) {
		$is_post_new = true;
		if( ! empty( $_GET['post'] ) ) {
			$is_post_new = false;
		}
		...
		<div id="post_types_list" class="meta_section">
			...
			<div>
				<label for="layout">Layout:</label><select
					...
					endforeach;
					?>
				</select> <a href="<?php echo admin_url('admin.php?page=placester_shortcodes_template_edit')?>">(create new)</a>
			</div>
		</div>
		<?php 
	...
	// add meta box for featured listings- adding custom fields
	public  function pl_widgets_template_meta_box_cb( $post ) {
		$is_post_new = true;
		if( ! empty( $_GET['post'] ) ) {
			$is_post_new = false;
		}
...
// 			}
	...
	public function shortcode_edit_link($url, $ID, $context) {
		global $pagenow;
		if ($pagenow == 'admin.php' && get_post_type($ID) == 'pl_general_widget') {
